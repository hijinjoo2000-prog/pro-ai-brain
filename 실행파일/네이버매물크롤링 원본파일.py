...
                
                # 대기 중 생존 신호 (2분 주기)
                self.log_signal.emit("⏳ 2분 후 다음 스캔을 시작합니다...")
                for _ in range(2 * 60):
                    if not self.running: return
                    time.sleep(1)
        except Exception as e:
...
                row = self._table_id_row_map[art_id]
            else:
                # 신규 행 추가
                row = self.table.rowCount()
                self.table.insertRow(row)
                if art_id:
                    self._table_id_row_map[art_id] = row
    
...
    
            self.table.setItem(row, 0, QTableWidgetItem(apt_text))
